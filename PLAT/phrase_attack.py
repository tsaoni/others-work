...
from transformers import RobertaForMaskedLM, RobertaTokenizer, AutoTokenizer, AutoModelForMaskedLM
...
if __name__ == "__main__":
    ...
    all_phrases, all_sentences, all_word_lists = examples2phrases(examples, "./stanford-corenlp-4.5.0", dataset)

    # prepare target classifier
    label_map = {"yelp": ["Negative", "Postive"],
    ...
    # BART = torch.hub.load('pytorch/fairseq', 'bart.base', source="local")
    ...
    ll_tokenizer = AutoTokenizer.from_pretrained(("./likelihood_model/%s" % args.dataset))
    ll_model = AutoModelForMaskedLM.from_pretrained(("./likelihood_model/%s" % args.dataset)).to("cuda").eval()
